	...
	<?elseif(count($arResult["SEARCH"])>0):?>
		...
		<?foreach($arResult["SEARCH"] as $arItem):?>
			<div class="search-item">
				<h4><a href="<?=SITE_DIR.$arItem["URL"]?>"><?echo $arItem["TITLE_FORMATED"]?></a></h4>
				...
				<div class="search-item-meta">
					...
					<?if($arParams["SHOW_ITEM_TAGS"] != "N" && !empty($arItem["TAGS"])):?>
						...
						foreach ($arItem["TAGS"] as $tags):
							?><a href="<?=SITE_DIR.$tags["URL"]?>"><?=$tags["TAG_NAME"]?></a> <?
						endforeach;
						?></div>
					<?endif;?>
		...
		<?if($arParams["SHOW_ORDER_BY"] != "N"):?>
			...
			<?if($arResult["REQUEST"]["HOW"]=="d"):?>
				<a href="<?=SITE_DIR.$arResult["URL"]?>&amp;how=r"><?=GetMessage("CT_BSP_ORDER_BY_RANK")?></a>&nbsp;<b><?=GetMessage("CT_BSP_ORDER_BY_DATE")?></b>
			<?else:?>
				<b><?=GetMessage("CT_BSP_ORDER_BY_RANK")?></b>&nbsp;<a href="<?=SITE_DIR.$arResult["URL"]?>&amp;how=d"><?=GetMessage("CT_BSP_ORDER_BY_DATE")?></a>
			<?endif;?>
			</div>
		<?endif;?>
	<?else:?>
